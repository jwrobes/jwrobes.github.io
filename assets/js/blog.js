				...
				if (checkIfNode(node)) {
					if (checkifBreaker(node)) {
						if (oneColumnOnly) {
							...
							setBreakerDivHeightMatch(node)
						}
						break
					}
					if (firstPostColumn0) {
						setPositionFirstPost(node)
					}
					if (firstPostColumn1) {
						setPositionSecondPost(node)
					}
					if (oneColumnOnly) {
...
var firstPostColumn1 = function (col0_y_position) {
